...
//   static final _logger = Yuro.tag('DiskLruCache');
...
//   static Future<DiskLruCache> open(Directory directory, [int? maxSize]) async {
...
//         _logger.e('$directory is dirty: ${e.toString()}, remove.');
//         cache.clear();
//       }
//     }
//     // 创建缓存目录
...
//       _logger.e('The size of cache space must be greater than zero.(maxSize): $maxSize');
//       maxSize = null;
//     }
//     _maxSize = maxSize ?? 100.MB;
...
//   /// 增加冗余计数器,根据需要重构日志文件
//   void _addRedundantOpCount() {
//     _redundantOpCount++;
//     if (_redundantOpCount >= 2000 || _redundantOpCount >= _lruEntries.length) {
//       _logger.i('redundantOpCount: $_redundantOpCount, cacheEntry count: ${_lruEntries.length}, 开始重构缓存日志文件');
//       _rebuildJournal();
//     }
//   }
...
//   Future _editCommit(Entry entry, bool success) async {
...
//     if (entry == null) {
//       _logger.i('cache key: $key 不存在.');
//       return null;
//     }
//     if (!entry.readable) {
//       _logger.i('cache key: $key 正在编辑中,无法读取.');
//       return null;
//     }
//     if (entry.isExpire()) {
//       _logger.i('cache key: $key 已过期,将从内存和本地移除.');
//       remove(key);
//       return null;
//     }
//     var expire = entry.expire;
...
//   /// 编辑缓存
//   Future<Editor> edit(String key, [Duration? expire]) async {
//     Entry entry = _lruEntries[key] ??= Entry(key, _directory);
//     if (expire != null) {
...
//     Entry? entry = _lruEntries[key];
//     if (entry == null) {
//       _logger.d('key: "$key"不存在,移除失败.');
//       return false;
//     }
//     if (!entry.readable) {
//       _logger.d('key: "$key"正在编辑中,无法移除.');
//       return false;
//     }
//     entry.getCleanFile().deleteIfExists();
//     _size -= entry.length;
