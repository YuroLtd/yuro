...
mixin UdpMixin {
  ...
  Future<void> startUdp(InternetAddress address, int port) async {
    ...
    Yuro.tag('Udp').i('udp server on [${address.address}:$port] start success.');
  }

  void _handlerEvent(RawSocketEvent event) {
    final datagram = _udpSocket?.receive();
    if (datagram == null) return;
    Yuro.tag('Udp').i('[${datagram.ap}]: ${datagram.data.format()}');
  ...
  Future<void> closeUdp() async {
    ...
    Yuro.tag('Udp').i('udp server closing.');
    await _subscription?.cancel();
    _udpSocket?.close();
    _udpSocket = null;
    Yuro.tag('Udp').i('udp server closed.');
  }
}
