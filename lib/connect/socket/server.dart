...
mixin SocketServer {
  ...
  Future<void> startServer(
    ...
    _server = server;
    _startHeart();
    Yuro.tag('SocketServer').info('socket server on [${server.ap}] started, listen for connections.');
  }
  ...
  void _handlerConnection(Socket socket) async {
    ...
    Yuro.tag('SocketServer').info('[${socket.remoteAp}] connect.');
  ...
  Future<void> closeServer() async {
    ...
    Yuro.tag('SocketServer').info('socket server closing.');
    _pingTimer?.cancel();
    _socketPool.values.forEachIndexed((index, socket) {
      add(socket, SocketFrame.disconnect('server closed'));
      Yuro.tag('SocketServer').info('close connection. [${index + 1}/$socketSize]');
      _closeConnection(socket.identityId);
    });
    await _server!.close();
    Yuro.tag('SocketServer').info('socket server closed.');
  }
  ...
  void _onData(Socket socket, Uint8List data) {
    ...
    switch (frame.opcode) {
      ...
      case SocketOpcode.text:
        Yuro.tag('SocketServer').info('[${socket.remoteAp}]: ${frame.toString()}');
      ...
      case SocketOpcode.disconnect:
        Yuro.tag('SocketServer').info('[${socket.remoteAp}] disconnect.');
        _closeConnection(socket.identityId);
        break;
      case SocketOpcode.invalid:
        break;
      case SocketOpcode.unknown:
        Yuro.tag('SocketServer').warning('${socket.identityId} unknown opcode(${frame.opcode})');
        break;
    }
  }
  ...
  bool add(Socket target, SocketFrame frame) {
    try {
      ...
      Yuro.tag('SocketServer').severe('[${target.identityId}] $e');
      return false;
    }
  }
  ...
  bool addBy(int identityId, SocketFrame frame) {
    ...
    if (target == null) {
      Yuro.tag('SocketServer').warning('target [$identityId] not exist.');
      return false;
    } else {
    ...
    pingFunc(Socket socket) {
      ...
      if (add(socket, SocketFrame.ping())) {
        _pongPool[identityId] = Timer(_pongTimeout, () {
          Yuro.tag('SocketServer').warning('[$ap] pong timeout, close connection.');
